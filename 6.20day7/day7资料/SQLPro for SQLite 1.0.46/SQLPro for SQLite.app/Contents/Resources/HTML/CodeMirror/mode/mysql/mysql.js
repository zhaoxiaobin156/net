...
CodeMirror.defineMode("mysql", function(config) {
  ...
  var baseKeywords = [
  	('ACCESSIBLE'),('AFTER'),('ALTER'),('AS'),('BEGIN'),('BEFORE'),('BINARY'),('BY'),('CASE'),('CHARACTER'),('COLUMN'),('CONTINUE'),('CROSS'),('CURRENT_TIMESTAMP'),('DATABASE'),('DAY_MICROSECOND'),('DEC'),('DEFAULT'),
	('DESC'),('DISTINCT'),('DOUBLE'),('EACH'),('ENCLOSED'),('END'),('EXIT'),('FETCH'),('FLOAT8'),('FOREIGN'),('GRANT'),('HIGH_PRIORITY'),('HOUR_SECOND'),('IN'),('INNER'),('INSERT'),('INT2'),('INT8'),
  ...
  keywords = wordRegexp(baseKeywords);

  tableKeywords = [];
  if(typeof extraKeywords == 'function') {
    tableKeywords = tableKeywords.concat(extraKeywords());
  }
  tableKeywords = wordRegexp(tableKeywords);
  ...
  function tokenBase(stream, state) {
    ...
    if (ch == "$" || ch == "?") {
      ...
      if (ops.test(word))
        ...
        return "keyword";
      else if (tableKeywords.test(word))
        return "table";
      else
        return "variable";
    }
